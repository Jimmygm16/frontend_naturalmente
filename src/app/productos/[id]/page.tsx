...
export default function SingleProductPage({ params }: { params: { id: string } }): JSX.Element {

  const [product, isLoading] = useFetch(`/products/${params.id}`) as [Product, boolean];
  ...
  return (
    <>
      { isLoading && <Loading /> }
      {product && <section className="h-screen grid grid-cols-2 py-3">
        ...
        <section className="flex flex-col w-[90%] mx-auto">
          ...
          <div className="flex flex-row gap-3 py-5">
            ...
            <button className="btn w-full bg-orange-300">
              Comprar
            </button>
          </div>
